...
export const SupplierList = ({ suppliers }: SupplierListProps) => {
  ...
  const getTypeColor = (type: string) => {
    switch (type.toLowerCase()) {
      case 'food': return 'bg-emerald-100 text-emerald-800';
      ...
      case 'office': return 'bg-slate-100 text-slate-800';
            ...
            >
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <Badge className={getFrequencyColor(supplier.payment_frequency)}>
                    {supplier.payment_frequency}
                  </Badge>
                </div>
              ...
              <div className="text-right">
                <div className="flex items-center gap-1 text-lg font-bold text-gray-900">
                  ...
                  {supplier.amount_per_period.toLocaleString('en-AU', { minimumFractionDigits: 2 })}
                </div>
                <p className="text-sm text-gray-600">per {supplier.payment_frequency}</p>
                <p className="text-xs text-gray-500 mt-1">
                  ${supplier.monthly_total.toLocaleString('en-AU')} / month
                </p>
              </div>
            </div>
          ))}
